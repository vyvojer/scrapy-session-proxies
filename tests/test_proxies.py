...
from scrapy_cookie_proxies.proxies import *
...
class ProxyListTest(unittest.TestCase):
    ...
    def test_live_proxies(self):
        self.proxy_list[0].dead = True
        self.assertEqual(len(self.proxy_list), 3)
        self.assertEqual(len(self.proxy_list.live_proxies), 2)

    def test_checked_proxies(self):
        self.proxy_list[0].checked = True
        self.proxy_list[1].checked = True
        self.assertEqual(len(self.proxy_list), 3)
        self.assertEqual(len(self.proxy_list.checked_proxies), 2)
